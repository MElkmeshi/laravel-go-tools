...
var setsIntersectCmd = &cobra.Command{
	Use:   "intersect",
	Short: "Return elements present in both sets",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		result := sets.Intersect(input.SetA, input.SetB)
		return gio.WriteOutput(map[string]any{"result": result})
	},
}

func init() {
	setsCmd.AddCommand(setsIntersectCmd)
}
